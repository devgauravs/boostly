...
import {
  historyService,
  pointLeaderBoardService,
  pointService,
  pointTrackingService,
  purchaseRewardsService,
  RewardsService,
  totalPointsService,
} from '../../services/RewardsService/rewardService';
...
export const purchaseRewards = createAsyncThunk(
  ...
  async ({ userId, rewardId }: PurchaseRewardsPayload, thunkAPI) => {
    ...
    try {
      const response = await purchaseRewardsService.purchaseRewards(
        userId,
        rewardId,
      );
      Toast.show({
...
export const fetchLeaderBoard = createAsyncThunk(
  ...
  async (period: '7days' | '30days' | 'alltime', { rejectWithValue }) => {
    try {
